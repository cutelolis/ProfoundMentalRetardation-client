...
public class AutismHudElementSettingsScreen extends Screen {
    ...
    private void row(GuiGraphicsExtractor graphics, Row row, int x, int y, int w, int mx, int my) {
        ...
        if (row.type == RowType.BOOL) {
            ...
            int border = row.key.equals(focusedKey) ? autismclient.util.AutismTheme.recolor(RED, autismclient.util.AutismTheme.Channel.OUTLINE) : THEME.borderSoft();
    ...
    private void renderSlider(GuiGraphicsExtractor graphics, Row row, int x, int y, int w, int h, double value, boolean editing) {
        ...
        int fieldY = y - 3;
        frame(graphics, x + sliderW + 7, fieldY, fieldW, FIELD_H, 0xFF120D10, editing ? autismclient.util.AutismTheme.recolor(RED, autismclient.util.AutismTheme.Channel.OUTLINE) : THEME.borderSoft());
